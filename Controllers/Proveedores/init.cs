...
namespace MyApiProject.Controllers.proveedores
{
    [ApiExplorerSettings(GroupName = "proveedores")]
    [Route("api/v1/proveedores")]
    [ApiController]
    public partial class ProveedoresController : BaseController
    {
        private readonly IMemoryCache _memoryCache;
        private readonly AuthUtils _authUtils;

        public ProveedoresController(IConfiguration configuration, IMemoryCache memoryCache, AuthUtils authUtils) : base(configuration, memoryCache)
        {
            _memoryCache = memoryCache;
            _authUtils = authUtils;
        }

        // ✅ Consulta proveedores (sin ID)
        [Authorize]
        [HttpGet("consultar")]
        public async Task<IActionResult> ConsultarProveedores()
        {
            ...
            string cacheKey = $"proveedores_all";
            ...
            await _authUtils.InsertUserHistory(userId, "proveedores load all", $"Consulta de todos los registros en proveedores");
            return Ok(results);
        }
        ...
        public async Task<IActionResult> ConsultarPorId(int id)
        {
            ...
            string cacheKey = $"proveedores_{id}";
            if (_memoryCache.TryGetValue(cacheKey, out List<Dictionary<string, object>> cachedResults))
                return Ok(cachedResults);
            ...
            _memoryCache.Set(cacheKey, results, TimeSpan.FromMinutes(5));
            await _authUtils.InsertUserHistory(userId, "proveedores load by id", $"Consulta en proveedores con ID {id}");
        ...
        public async Task<IActionResult> ConsultarProveedoresConFiltros(
                    ...
                    [FromQuery] int pageSize = 10)
        {
            ...
            var baseQuery = @"FROM proveedores";
            ...
            try
            {
                ...
                var cacheKey = $"proveedores_filtros_{filtrosCacheKey}_page{page}_size{pageSize}";
                _memoryCache.Set(cacheKey, results, TimeSpan.FromMinutes(5));

                await _authUtils.InsertUserHistory(userId, "proveedores load with filters",
                    $"Consulta en proveedores con {request.Filtros.Count} filtros, página {page}, tamaño {pageSize}");
            ...
            {
                await _authUtils.InsertUserHistory(userId, "proveedores error",
        ...
        public async Task<IActionResult> Registrar([FromBody] JObject data)
        {
            ...
            var query = $@"
            INSERT INTO [proveedores] ({columnNames})
            OUTPUT INSERTED.id
            ...
            if (insertedId != null)
            {
                await _authUtils.InsertUserHistory(userId, "proveedores insert", $"Registro en proveedores con ID {insertedId}");
                _memoryCache.Remove($"proveedores_all");
            }
        ...
        public async Task<IActionResult> Actualizar(int id, [FromBody] JObject data)
        {
            ...
            string query = $"UPDATE proveedores SET {setClause} WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "proveedores update", $"Actualización en proveedores con ID {id}");
                _memoryCache.Remove($"proveedores_{id}");
                _memoryCache.Remove($"proveedores_all");
        ...
        public async Task<IActionResult> Archivar(int id)
        {
            ...
            string query = $"UPDATE proveedores SET estado = 'archivado' WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "proveedores delete", $"Archivado de documento en proveedores con ID {id}");
                _memoryCache.Remove($"proveedores_{id}");
                _memoryCache.Remove($"proveedores_all");
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            ...
            string query = $"DELETE proveedores WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "proveedores delete", $"Eliminación en proveedores con ID {id}");
                _memoryCache.Remove($"proveedores_{id}");
                _memoryCache.Remove($"proveedores_all");
                return Ok(new { Message = "Registro eliminado exitosamente" });
            }
