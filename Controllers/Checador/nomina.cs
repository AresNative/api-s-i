...
namespace MyApiProject.Controllers.checador
{
    ...
    [Route("api/v1/nomina")]
    [ApiController]
    public partial class NominaController : BaseController
    {
        private readonly IMemoryCache _memoryCache;
        private readonly AuthUtils _authUtils;

        public NominaController(IConfiguration configuration, IMemoryCache memoryCache, AuthUtils authUtils)
        ...
        public async Task<IActionResult> ConsultarNomina()
        {
            ...
            await _authUtils.InsertUserHistory(userId, "nomina load all", $"Consulta de todos los registros en nomina");
            return Ok(results);
        }
        ...
        public async Task<IActionResult> ConsultarPorId(int id)
        {
            ...
            string cacheKey = $"nomina_{id}";
            if (_memoryCache.TryGetValue(cacheKey, out List<Dictionary<string, object>> cachedResults))
                return Ok(cachedResults);

            string query = $"SELECT * FROM nomina WHERE id = @ID";
            ...
            _memoryCache.Set(cacheKey, results, TimeSpan.FromMinutes(5));
            await _authUtils.InsertUserHistory(userId, "nomina load by id", $"Consulta en nomina con ID {id}");
        ...
        public async Task<IActionResult> Registrar([FromBody] JObject data)
        {
            ...
            var query = $@"
            INSERT INTO [nomina] ({columnNames})
            OUTPUT INSERTED.id
            ...
            if (insertedId != null)
            {
                await _authUtils.InsertUserHistory(userId, "nomina insert", $"Registro en nomina con ID {insertedId}");
                _memoryCache.Remove($"nomina_all");
            }
        ...
        public async Task<IActionResult> Actualizar(int id, [FromBody] JObject data)
        {
            ...
            string query = $"UPDATE nomina SET {setClause} WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "nomina update", $"Actualización en nomina con ID {id}");
                _memoryCache.Remove($"nomina_{id}");
                _memoryCache.Remove($"nomina_all");
        ...
        public async Task<IActionResult> Archivar(int id)
        {
            ...
            string query = $"UPDATE nomina SET estado = 'archivado' WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "nomina delete", $"Archivado de documento en nomina con ID {id}");
                _memoryCache.Remove($"nomina_{id}");
                _memoryCache.Remove($"nomina_all");
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            ...
            string query = $"DELETE nomina WHERE id = @Id";
            ...
            if (result > 0)
            {
                await _authUtils.InsertUserHistory(userId, "nomina delete", $"Eliminación en nomina con ID {id}");
                _memoryCache.Remove($"nomina_{id}");
                _memoryCache.Remove($"nomina_all");
                return Ok(new { Message = "Registro eliminado exitosamente" });
            }
